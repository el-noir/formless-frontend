        ...
        <div className="grid md:grid-cols-3 gap-8 relative w-full">
          ...
          <div className="hidden md:block absolute top-[180px] left-[16%] right-[16%] h-px bg-gray-800 z-0">
            <motion.div
              ...
              className="h-full w-1/3 bg-brand-purple opacity-50"
            />
          </div>
          ...
          {steps.map((step, index) => (
            ...
            >
              <div className="w-full h-[320px] bg-brand-dark border border-gray-800 rounded-xl overflow-hidden relative group-hover:border-gray-600 transition-all duration-300 flex flex-col">

                {/* Visual Area */}
                <div className="flex-1 relative bg-brand-surface flex items-center justify-center p-6">
                  {(() => {
                ...
                {/* Step Number Badge */}
                <div className="absolute top-6 left-6 w-8 h-8 rounded-md bg-brand-surface border border-gray-800 flex items-center justify-center text-xs font-bold text-white group-hover:bg-[#1C1C22] transition-colors duration-300">
                  {step.id}
                </div>

                {/* Text Content */}
                <div className="p-6 bg-brand-dark border-t border-gray-800 relative z-20">
